...
public class GithubApiServiceImpl {
    JsonParseServiceImpl jsonParse;
    public String callAPI(String userId) throws JsonProcessingException {

        HashMap<String, Object> result = new HashMap<String, Object>();

        String jsonInString = "";

        UserGitRepository git;
...
//                .queryParam("t_invoice", trackingNo)
            ...
            ResponseEntity<String> resultMap = restTemplate
                .exchange(uri.toString(), HttpMethod.GET, entity, String.class);

            result.put("statusCode", resultMap.getStatusCodeValue()); //http status code를 확인
            result.put("header", resultMap.getHeaders()); //헤더 정보 확인
            result.put("body", resultMap.getBody()); //실제 데이터 정보 확인
            jsonParse.getRepoList(resultMap.getBody());
